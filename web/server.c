...
" idx.forEach((s,k)=>{x.strokeStyle=col(k,idx.length);x.lineWidth=2;x.beginPath();let started=false;\n"
"  for(let a=0;a<P.twa.length;a++){const b=P.bsp[a][s];if(b<=0){started=false;continue;}const p=px(P.twa[a],b);\n"
"   started?x.lineTo(p[0],p[1]):x.moveTo(p[0],p[1]);started=true;}x.stroke();});\n"
" x.lineWidth=1;\n"
