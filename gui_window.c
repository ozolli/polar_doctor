...
void on_help_clicked(GtkWidget *widget, gpointer user_data) {
    ...
    const char *help_text_fr =
        ...
        "<b>• Ouvrir ▾</b>\n"
        "  Menu des <b>bateaux récents</b> + « Parcourir un bateau… »\n"
        "  Ouvre aussi un simple fichier polaire (.pol) ; confirmation si modifications non sauvegardées.\n\n"
        "<b>• Nouveau bateau</b>\n"
        "  Crée un dossier bateau et sa configuration (boat.cfg).\n\n"
        "<b>• Bateau…</b>\n"
        "  Éditeur graphique de l'<b>inventaire</b> du bateau, enregistré en boat.cfg (INI) :\n"
        ...
        "  - <b>polaires multiples</b> : définir N polaires et, pour chacune, les voiles et états\n"
        "    de mer concernés via un pop-up à cases à cocher. À la création/mise à jour, chaque\n"
        "    point est routé vers les polaires dont les critères correspondent (un .pol par polaire).\n\n"
        "<b>• Sélecteur de polaire</b> (barre latérale)\n"
        "  Bascule entre les polaires du bateau ouvert.\n\n"
        ...
        "L'état voile / moteur / mer est piloté en direct par des <b>listes déroulantes + bouton\n"
        "Moteur</b> (les commentaires VDR sont ignorés en live) ; chaque point est routé vers les\n"
        "polaires correspondantes. Visualisation temps réel : nuage de points bruts + point courant,\n"
        "échelle et plage TWS qui suivent les données. Les grilles sont ré-ensemencées depuis les\n"
        ".pol au démarrage et enregistrées à l'arrêt.\n\n"
        "<b>FORMAT DES FICHIERS</b>\n\n"
        "<b>• Fichiers NMEA (.txt, .nmea, .log)</b>\n"
        "  Vent : MWV (apparent/vrai) ou MWD (vent vrai) ; vitesse surface : VHW\n"
        "  Cap (pour MWD → TWA) : HDT, HDG ou le champ cap de VHW\n"
        "  Unités de vent reconnues : nœuds, m/s, km/h\n"
    ...
    const char *help_text_en =
        ...
        "<b>• Open ▾</b>\n"
        "  Menu of <b>recent boats</b> + \"Browse a boat…\"\n"
        "  Also opens a plain polar file (.pol); confirmation if there are unsaved changes.\n\n"
        "<b>• New boat</b>\n"
        "  Creates a boat folder and its configuration (boat.cfg).\n\n"
        "<b>• Boat…</b>\n"
        "  Graphical editor of the boat <b>inventory</b>, saved as boat.cfg (INI):\n"
        ...
        "  - <b>multiple polars</b>: define N polars and, for each, the sails and sea states\n"
        "    via a checkbox pop-up. On create/update, every point is routed to the polars whose\n"
        "    criteria match (one .pol per polar).\n\n"
        "<b>• Polar selector</b> (sidebar)\n"
        "  Switch between the open boat's polars.\n\n"
        "<b>MAIN BUTTONS</b>\n\n"
        "<b>• Open</b>\n"
        ...
        "Real-time view: raw point cloud + current point, scale and TWS range follow the data.\n"
        "Grids are re-seeded from the .pol at start and saved on stop.\n\n"
        "<b>FILE FORMATS</b>\n\n"
        "<b>• NMEA files (.txt, .nmea, .log)</b>\n"
        "  Wind: MWV (apparent/true) or MWD (true wind); speed through water: VHW\n"
        "  Heading (for MWD → TWA): HDT, HDG or the VHW heading field\n"
        "  Recognized wind units: knots, m/s, km/h\n"
