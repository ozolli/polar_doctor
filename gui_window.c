...
void on_help_clicked(GtkWidget *widget, gpointer user_data) {
    ...
    const char *help_text_fr =
        "<big><b>Polar Doctor — Guide d'utilisation</b></big>\n"
        "\n"
        "<b>À QUOI SERT CE PROGRAMME ?</b>\n"
        "\n"
        "Polar Doctor construit la « polaire » de votre voilier : un tableau, et un\n"
        "diagramme, qui donne la vitesse du bateau pour chaque combinaison d'angle et\n"
        "de force du vent. C'est la carte d'identité de performance du bateau, utilisée\n"
        "par les logiciels de routage et pour comparer/régler vos navigations.\n"
        "\n"
        "Le programme fabrique cette polaire à partir de vos données <b>réelles</b> :\n"
        "  • fichiers de log NMEA0183 (issus de l'électronique du bord),\n"
        "  • bases VDR de qtVlm (enregistrements de navigation),\n"
        "  • ou en direct (« capture live ») pendant que vous naviguez.\n"
        "Plus vous accumulez de données dans des conditions variées, plus la polaire est fidèle.\n"
        "\n"
        "<b>LE VOCABULAIRE EN 30 SECONDES</b>\n"
        "\n"
        "  • <b>TWA</b> (True Wind Angle) — angle du vent réel par rapport à l'axe du\n"
        "    bateau, de 0° (vent debout) à 180° (vent arrière).\n"
        "  • <b>TWS</b> (True Wind Speed) — force du vent réel, en nœuds.\n"
        "  • <b>STW / BSP</b> (Speed Through Water) — vitesse du bateau dans l'eau (loch).\n"
        "    C'est ce que la polaire cherche à représenter.\n"
        "  • <b>SOG</b> (Speed Over Ground) — vitesse par rapport au fond (GPS) ; sert à\n"
        "    fiabiliser le STW.\n"
        "  • <b>VMG</b> (Velocity Made Good) — composante de la vitesse dans l'axe du\n"
        "    vent ; mesure l'efficacité au près et au portant.\n"
        "  • <b>Vent apparent</b> (AWA / AWS) — le vent ressenti à bord (vent réel combiné\n"
        "    à la vitesse du bateau).\n"
        "\n"
        "<b>PRISE EN MAIN — 4 ÉTAPES</b>\n"
        "\n"
        "  <b>1.</b> Ouvrez ou créez un bateau : « <b>Nouveau bateau</b> », ou « <b>Ouvrir ▾</b> »\n"
        "     pour un bateau récent. Un bateau est un <i>dossier</i> qui regroupe sa\n"
        "     configuration et ses polaires.\n"
        "  <b>2.</b> (Facultatif) Réglez l'inventaire du bateau (« <b>Bateau…</b> ») : noms de\n"
        "     vos voiles, états de mer, et nombre de polaires souhaité.\n"
        "  <b>3.</b> Fabriquez la polaire :\n"
        "       • « <b>Créer</b> » à partir de fichiers de log (NMEA / VDR), ou\n"
        "       • « <b>Capture live</b> » pendant que vous naviguez.\n"
        "  <b>4.</b> Lisez le résultat (onglets <b>Données</b> / <b>Polaire</b> / <b>VMG</b>),\n"
        "     corrigez si besoin, puis « <b>Export PDF</b> ».\n"
        "\n"
        "Vous pouvez aussi simplement ouvrir un fichier .pol existant pour le consulter.\n"
        "\n"
        "<b>MODE BATEAU</b>\n"
        "\n"
        "Un bateau = un <b>dossier</b> contenant sa configuration et ses polaires.\n"
        "La barre latérale affiche le bateau ouvert, la liste de ses polaires et les actions.\n"
        "\n"
        "  • <b>Ouvrir ▾</b> — menu des <b>bateaux récents</b> + « Parcourir un bateau… ».\n"
        "    Ouvre aussi un simple fichier polaire (.pol) ; confirmation si modifications\n"
        "    non sauvegardées.\n"
        "  • <b>Nouveau bateau</b> — crée un dossier bateau et sa configuration (boat.cfg).\n"
        "  • <b>Bateau…</b> — éditeur graphique de l'<b>inventaire</b>, enregistré en boat.cfg :\n"
        "       - grand-voile (haute, 1 ris, 2 ris…), voiles d'avant (J1, spi…)\n"
        "       - états de mer (échelle Douglas/OMM par défaut, libellés FR/EN, éditables)\n"
        "       - mots-clés moteur (« Moteur » exclut, « Charge » garde — hélice débrayée)\n"
        "       - <b>polaires multiples</b> : définir N polaires et, pour chacune, les voiles\n"
        "         et états de mer concernés via un pop-up à cases à cocher. À la\n"
        "         création/mise à jour, chaque point est routé vers les polaires dont les\n"
        "         critères correspondent (un .pol par polaire).\n"
        "  • <b>Sélecteur de polaire</b> (barre latérale) — bascule entre les polaires du bateau.\n"
        "\n"
        "<b>FABRIQUER UNE POLAIRE</b>\n"
        "\n"
        "  • <b>Créer</b> — génère une nouvelle polaire à partir de fichiers NMEA ou VDR.\n"
        "       - sélection multiple possible (Ctrl+clic ou Maj+clic) ; .txt/.nmea/.log (NMEA) et .db (VDR)\n"
        "       - tous les fichiers sont traités et combinés\n"
        "       - la progression s'affiche pendant le traitement\n"
        "  • <b>Mettre à jour</b> — ajoute de nouvelles données à la polaire ouverte.\n"
        "       - données existantes et nouvelles <b>ré-agrégées</b> : la polaire peut monter\n"
        "         OU baisser (utile pour adapter une polaire théorique VPP au bateau réel).\n"
        "\n"
        "  <b>Comment c'est calculé (les coulisses) :</b>\n"
        "       - les points sont groupés par cases de 5° (TWA) et 2 nœuds (TWS)\n"
        "       - minimum 3 points par case\n"
        "       - on retient un <b>percentile élevé</b> (P90 par défaut, réglable P85–P95 via\n"
        "         le curseur de la barre d'outils) : on vise la performance <i>atteignable</i>,\n"
        "         pas la moyenne (le haut percentile écarte les ralentissements : virements, auloffées)\n"
        "       - STW lissé (loch bruité) à l'import NMEA, puis débruité par comparaison au SOG\n"
        "         (les sauts du loch — coque déjaugée, roue à aube bloquée — sont rejetés)\n"
        "       - filtre moteur : points sous moteur exclus, sauf charge batteries (hélice débrayée)\n"
        "\n"
        "<b>CAPTURE LIVE</b> (colonne à droite du diagramme)\n"
        "\n"
        "Alimente les polaires du bateau en temps réel depuis 3 sources :\n"
        "  • <b>VDR qtVlm</b> — suit le fichier vdr.db en lecture seule (mode WAL)\n"
        "  • <b>NMEA TCP</b> — client vers une passerelle (ex. hôte:10110)\n"
        "  • <b>NMEA UDP</b> — écoute un port de diffusion\n"
        "L'état voile / moteur / mer est piloté en direct par des <b>listes déroulantes +\n"
        "bouton Moteur</b> (les commentaires VDR sont ignorés en live) ; chaque point est\n"
        "routé vers les polaires correspondantes. Visualisation temps réel : nuage de\n"
        "points bruts + point courant, échelle et plage TWS qui suivent les données.\n"
        "Les grilles sont ré-ensemencées depuis les .pol au démarrage, enregistrées à l'arrêt.\n"
        "\n"
        "<b>LIRE ET MODIFIER LA POLAIRE</b>\n"
        "\n"
        "  • <b>Onglet Données</b> — tableau des vitesses (BSP) selon TWA (lignes) et TWS\n"
        "    (colonnes). Double-cliquer sur une cellule pour modifier la valeur.\n"
        "  • <b>Onglet Polaire</b> — le diagramme. Choisir la plage de TWS à afficher\n"
        "    (de / à) ; une couleur par TWS + légende ; courbes lissées (Catmull-Rom) ;\n"
        "    vert = plage VMG utile, rouge = VMG dégradé (près trop serré / portant trop bas).\n"
        "       <b>Mode dynamique</b> (case à cocher) : saisir une TWS (boutons ±1 kn,\n"
        "       décimales possibles) pour n'afficher que sa courbe ; clic maintenu/glissé\n"
        "       sur le diagramme → lecture en direct TWA / AWA / AWS / BS / VMG ; au\n"
        "       relâchement, vitesses max de la courbe et de la polaire.\n"
        "  • <b>Onglet VMG</b> — meilleurs angles au près (VMG up) et au portant (VMG down)\n"
        "    pour chaque TWS.\n"
        "  • <b>Ajout TWA / Ajout TWS</b> — insère une ligne d'angle ou une colonne de vent\n"
        "    (ordre croissant, cellules à 0.00).\n"
        "  • <b>Suppression</b> — cliquer le bouton puis l'en-tête de ligne/colonne à\n"
        "    supprimer (confirmation demandée).\n"
        "  • <b>Export PDF</b> — tableau + diagramme + VMG ; nom suggéré automatiquement ;\n"
        "    taille du texte ajustable via la variable POLAR_PRINT_SCALE (0.5 à 3.0).\n"
        "\n"
        "<b>FORMATS DE FICHIERS</b>\n"
        "\n"
        "  • <b>NMEA (.txt, .nmea, .log)</b> — vent : MWV (apparent/vrai) ou MWD (vent vrai) ;\n"
        "    vitesse surface : VHW ; cap (pour MWD → TWA) : HDT, HDG ou le champ cap de VHW ;\n"
        "    unités de vent reconnues : nœuds, m/s, km/h ; SOG lu s'il est présent\n"
        "    (RMC, VTG, VBW, RMA, OSD) pour débruiter le STW.\n"
        "  • <b>VDR (.db)</b> — base SQLite de qtVlm (TWA, TWS, STW ; SOG, RPM, COMMENT si présents).\n"
        "  • <b>Polaire (.pol)</b> — format tabulaire (séparateur point-virgule) : 1re ligne =\n"
        "    en-têtes TWS, colonnes suivantes = TWA + vitesses BSP.\n"
        "\n"
        "<b>CONSEILS</b>\n"
        "\n"
        "  • Combinez plusieurs sorties, dans des conditions variées, pour plus de précision.\n"
        "  • Utilisez « Mettre à jour » pour affiner progressivement une polaire.\n"
        "  • Un minimum de 3 points par case est requis : une case vide = pas assez de données.\n"
        "  • Le STW (loch) est d'autant plus fiable que le SOG (GPS) est disponible.\n";

    const char *help_text_en =
        "<big><b>Polar Doctor — User Guide</b></big>\n"
        "\n"
        "<b>WHAT IS THIS PROGRAM FOR?</b>\n"
        "\n"
        "Polar Doctor builds your sailboat's \"polar\": a table, and a diagram, giving the\n"
        "boat's speed for every combination of wind angle and wind strength. It is the\n"
        "boat's performance fingerprint, used by routing software and to compare/tune\n"
        "your sailing.\n"
        "\n"
        "The program builds this polar from your <b>real</b> data:\n"
        "  • NMEA0183 log files (from the onboard electronics),\n"
        "  • qtVlm VDR databases (sailing recordings),\n"
        "  • or live, while you are sailing.\n"
        "The more data you gather in varied conditions, the more accurate the polar.\n"
        "\n"
        "<b>VOCABULARY IN 30 SECONDS</b>\n"
        "\n"
        "  • <b>TWA</b> (True Wind Angle) — true wind angle relative to the boat's axis,\n"
        "    from 0° (head to wind) to 180° (dead downwind).\n"
        "  • <b>TWS</b> (True Wind Speed) — true wind strength, in knots.\n"
        "  • <b>STW / BSP</b> (Speed Through Water) — boat speed through the water (log).\n"
        "    This is what the polar represents.\n"
        "  • <b>SOG</b> (Speed Over Ground) — speed relative to the ground (GPS); used to\n"
        "    make STW more reliable.\n"
        "  • <b>VMG</b> (Velocity Made Good) — speed component along the wind axis;\n"
        "    measures upwind and downwind efficiency.\n"
        "  • <b>Apparent wind</b> (AWA / AWS) — the wind felt aboard (true wind combined\n"
        "    with boat speed).\n"
        "\n"
        "<b>GETTING STARTED — 4 STEPS</b>\n"
        "\n"
        "  <b>1.</b> Open or create a boat: \"<b>New boat</b>\", or \"<b>Open ▾</b>\" for a recent\n"
        "     boat. A boat is a <i>folder</i> holding its configuration and its polars.\n"
        "  <b>2.</b> (Optional) Set the boat inventory (\"<b>Boat…</b>\"): your sail names, sea\n"
        "     states, and how many polars you want.\n"
        "  <b>3.</b> Build the polar:\n"
        "       • \"<b>Create</b>\" from log files (NMEA / VDR), or\n"
        "       • \"<b>Live capture</b>\" while sailing.\n"
        "  <b>4.</b> Read the result (<b>Data</b> / <b>Polar</b> / <b>VMG</b> tabs), edit if\n"
        "     needed, then \"<b>Export PDF</b>\".\n"
        "\n"
        "You can also simply open an existing .pol file to view it.\n"
        "\n"
        "<b>BOAT MODE</b>\n"
        "\n"
        "A boat = a <b>folder</b> holding its configuration and its polars.\n"
        "The sidebar shows the open boat, its list of polars and the boat actions.\n"
        "\n"
        "  • <b>Open ▾</b> — menu of <b>recent boats</b> + \"Browse a boat…\". Also opens a\n"
        "    plain polar file (.pol); confirmation if there are unsaved changes.\n"
        "  • <b>New boat</b> — creates a boat folder and its configuration (boat.cfg).\n"
        "  • <b>Boat…</b> — graphical editor of the <b>inventory</b>, saved as boat.cfg:\n"
        "       - mainsail (full, 1 reef, 2 reefs…), headsails (J1, spinnaker…)\n"
        "       - sea states (Douglas/WMO scale by default, FR/EN labels, editable)\n"
        "       - engine keywords (\"Moteur\" excludes, \"Charge\" keeps — propeller disengaged)\n"
        "       - <b>multiple polars</b>: define N polars and, for each, the sails and sea\n"
        "         states via a checkbox pop-up. On create/update, every point is routed to\n"
        "         the polars whose criteria match (one .pol per polar).\n"
        "  • <b>Polar selector</b> (sidebar) — switch between the boat's polars.\n"
        "\n"
        "<b>BUILDING A POLAR</b>\n"
        "\n"
        "  • <b>Create</b> — generate a new polar from NMEA or VDR files.\n"
        "       - multiple selection (Ctrl+click or Shift+click); .txt/.nmea/.log (NMEA) and .db (VDR)\n"
        "       - all files are processed and combined\n"
        "       - progress is shown during processing\n"
        "  • <b>Update</b> — add new data to the open polar.\n"
        "       - existing and new data are <b>re-aggregated</b>: the polar can rise OR fall\n"
        "         (handy to adapt a theoretical VPP polar to the real boat).\n"
        "\n"
        "  <b>How it is computed (behind the scenes):</b>\n"
        "       - points are grouped into 5° (TWA) and 2 kn (TWS) buckets\n"
        "       - minimum 3 points per bucket\n"
        "       - a <b>high percentile</b> is kept (P90 by default, adjustable P85–P95 via the\n"
        "         toolbar slider): targets <i>achievable</i> performance, not the average\n"
        "         (the high percentile drops slowdowns: tacks, luffing)\n"
        "       - STW smoothed (noisy log) on NMEA import, then denoised against SOG\n"
        "         (log spikes — hull lifting, blocked paddle wheel — are rejected)\n"
        "       - engine filter: motoring points excluded, except battery charging (propeller disengaged)\n"
        "\n"
        "<b>LIVE CAPTURE</b> (column to the right of the diagram)\n"
        "\n"
        "Feeds the boat's polars in real time from 3 sources:\n"
        "  • <b>qtVlm VDR</b> — follows the vdr.db file read-only (WAL mode)\n"
        "  • <b>NMEA TCP</b> — client to a gateway (e.g. host:10110)\n"
        "  • <b>NMEA UDP</b> — listens on a broadcast port\n"
        "Sail / engine / sea state is driven live by <b>drop-down lists + a Motor button</b>\n"
        "(VDR comments are ignored in live); each point is routed to the matching polars.\n"
        "Real-time view: raw point cloud + current point, scale and TWS range follow the\n"
        "data. Grids are re-seeded from the .pol at start and saved on stop.\n"
        "\n"
        "<b>READING AND EDITING THE POLAR</b>\n"
        "\n"
        "  • <b>Data tab</b> — table of speeds (BSP) by TWA (rows) and TWS (columns).\n"
        "    Double-click a cell to edit the value.\n"
        "  • <b>Polar tab</b> — the diagram. Choose the TWS range to display (from / to);\n"
        "    one color per TWS + legend; curves smoothed (Catmull-Rom); green = useful VMG\n"
        "    range, red = degraded VMG (too tight upwind / too low downwind).\n"
        "       <b>Dynamic mode</b> (checkbox): enter a TWS (± buttons step 1 kn, decimals\n"
        "       allowed) to show only its curve; press and hold / drag on the diagram →\n"
        "       live readout of TWA / AWA / AWS / BS / VMG; on release, curve and polar max speeds.\n"
        "  • <b>VMG tab</b> — best upwind (VMG up) and downwind (VMG down) angles per TWS.\n"
        "  • <b>Add TWA / Add TWS</b> — insert an angle row or a wind column (ascending\n"
        "    order, cells set to 0.00).\n"
        "  • <b>Delete</b> — click the button then the row/column header to remove\n"
        "    (confirmation requested).\n"
        "  • <b>Export PDF</b> — table + diagram + VMG; filename suggested automatically;\n"
        "    text size adjustable via the POLAR_PRINT_SCALE variable (0.5 to 3.0).\n"
        "\n"
        "<b>FILE FORMATS</b>\n"
        "\n"
        "  • <b>NMEA (.txt, .nmea, .log)</b> — wind: MWV (apparent/true) or MWD (true wind);\n"
        "    speed through water: VHW; heading (for MWD → TWA): HDT, HDG or the VHW heading\n"
        "    field; recognized wind units: knots, m/s, km/h; SOG read when present\n"
        "    (RMC, VTG, VBW, RMA, OSD) to denoise STW.\n"
        "  • <b>VDR (.db)</b> — qtVlm SQLite database (TWA, TWS, STW; SOG, RPM, COMMENT if present).\n"
        "  • <b>Polar (.pol)</b> — tabular format (semicolon separator): first row = TWS\n"
        "    headers, following columns = TWA + BSP speeds.\n"
        "\n"
        "<b>TIPS</b>\n"
        "\n"
        "  • Combine several outings, in varied conditions, for better accuracy.\n"
        "  • Use \"Update\" to progressively refine a polar.\n"
        "  • A minimum of 3 points per bucket is required: an empty cell = not enough data.\n"
        "  • STW (log) is all the more reliable when SOG (GPS) is available.\n";
